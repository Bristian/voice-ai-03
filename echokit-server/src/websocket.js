...
class CallSession {
  ...
  async _saveSession(durationMs) {
    ...
    const url = `${config.AGENT_API_URL}/v1/sessions`;
    const body = {
    ...
    try {
      this.log.info({ url, turns: this.turnCount }, "Saving call session…");
      ...
      if (resp.ok) {
        this.log.info("Call session saved successfully");
      } else {
        const errText = await resp.text().catch(() => "");
        this.log.warn({ status: resp.status, body: errText.slice(0, 300) }, "Session save failed");
      }
    } catch (err) {
