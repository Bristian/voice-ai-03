...
# CORS — allow dashboard to call agent-api from the browser
...
async def save_session(request: SaveSessionRequest):
    """Save or update a call session. Called by echokit-server when a call ends."""
    from datetime import datetime, timezone
    from app.db import execute

    # Parse ISO datetime strings to Python datetime objects (asyncpg requires this)
    started = datetime.fromisoformat(request.started_at.replace("Z", "+00:00")) if request.started_at else datetime.now(timezone.utc)
    ended = datetime.fromisoformat(request.ended_at.replace("Z", "+00:00")) if request.ended_at else None

    await execute(
        """INSERT INTO call_sessions (vonage_call_id, caller_number, started_at, ended_at, transcript, entities, outcome)
           VALUES ($1, $2, $3, $4, $5, $6::jsonb, $7)
           ON CONFLICT (vonage_call_id) DO UPDATE SET
             ended_at = EXCLUDED.ended_at,
             transcript = EXCLUDED.transcript,
             entities = EXCLUDED.entities,
             outcome = EXCLUDED.outcome""",
        request.call_uuid,
        request.caller_phone_masked,
        started,
        ended,
        request.transcript,
        json.dumps(request.entities) if isinstance(request.entities, dict) else "{}",
        request.outcome,
    )
    return {"status": "saved"}
...
async def list_sessions(limit: int = 20, offset: int = 0):
    """List recent call sessions for the dashboard."""
    from app.db import fetch_all
    rows = await fetch_all(
        """SELECT vonage_call_id, caller_number, started_at, ended_at,
                  transcript, entities, outcome
           FROM call_sessions
    ...
    for r in rows:
        sessions.append({
            "call_uuid": r.get("vonage_call_id"),
            "caller_phone": r.get("caller_number", ""),
...
@app.get("/v1/sessions/{call_uuid}")
async def get_session(call_uuid: str):
    """Get a single call session by UUID."""
    from app.db import fetch_one
    r = await fetch_one(
        "SELECT * FROM call_sessions WHERE vonage_call_id = $1", call_uuid
    )
    if not r:
