...
async def process_turn(request: AgentTurnRequest) -> AgentTurnResponse:
    ...
    if intent in VEHICLE_INTENTS:
        ...
        # KNOWLEDGE TRACK: RAG search for FAQ/policy/financing answers
        t0 = time.monotonic()
        ...
        # If RAG doesn't have a good answer, DON'T transfer — just let synthesis
        # handle it. The synthesis prompt says "if you can't answer, say let me
        # get someone who can help" which is better than a hard transfer.
        if not is_grounded(rag_chunks):
            logger.info("RAG not grounded — synthesis will handle gracefully")
            rag_chunks = []  # Clear low-quality chunks so synthesis doesn't hallucinate
