...
async def retrieve_chunks(
    ...
    return await retrieve_chunks_with_embedding(query, query_vec, source_filter, top_k)


async def retrieve_chunks_with_embedding(
    query: str,
    query_vec: list[float] | None = None,
    source_filter: Sequence[str] | None = None,
    top_k: int | None = None,
) -> list[dict]:
    """Search pgvector with a pre-computed embedding. Embeds on-the-fly if vec is None.

    This is the inner function that agent.py calls when it has already started
    embedding in parallel with intent classification.
    """
    ...
    # If no pre-computed embedding, compute now
    if query_vec is None:
        query_vec = await embed_single(query)

    # Convert to numpy array — pgvector's asyncpg codec expects this format
    import numpy as np
    vec_array = np.array(query_vec, dtype=np.float32)
    ...
    if source_filter:
        ...
        params: list = [vec_array, *source_filter]
    else:
        where_clause = ""
        params = [vec_array]
