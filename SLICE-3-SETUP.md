...
### 6. Fixed follow-up question misclassification

Previously, saying "something under thirty thousand" after asking about SUVs would be classified as a knowledge/pricing intent instead of staying on the vehicle search track. The intent prompt now explicitly instructs the LLM: "if prior entities contain vehicle fields AND the new utterance refines that search, keep the intent as vehicle_search."

### 7. Fixed false transfers on FAQ questions

The hallucination guard was too aggressive — it triggered transfers when RAG found chunks but their cosine similarity scores were below the threshold. Two fixes:
- Lowered the threshold from 0.35 to 0.20 (appropriate for cosine distance without the cross-encoder reranker)
- Changed the behavior: instead of immediately transferring when RAG isn't grounded, the system now lets the synthesis LLM handle it gracefully ("let me get someone who can help with that" is a more natural response than a hard transfer)

### 8. Faster synthesis (GPT-4o-mini)

Switched the default synthesis model from GPT-4o to GPT-4o-mini. This cuts synthesis latency by ~60% (from ~400-700ms to ~150-250ms) with only a minor quality reduction. You can switch back to GPT-4o by setting `SYNTHESIS_MODEL=gpt-4o` in agent-api's environment variables.

### 9. Better error messages

When the agent-api call fails, the caller now hears "I apologize for the delay. Could you repeat your question?" instead of "I had trouble understanding that" — the issue is usually a timeout or network error, not a comprehension problem.
